...
const init = () => {
    ...
    // agregar el manejador de evento a la forma
...
function submitForm(evento) {
    ...
    // de la forma, obtenemos el input
    const inputEquivalencia = forma.elements.equivalencia;

    // del input, obtenemos su valor...
    const valorInput = inputEquivalencia.value;

    // y ahora le damos ese valor a la variable global
    monedaEquivalencia = valorInput;
    ...
    respuestaJson.then((respData) => {
        ...
        const datoEnHtml = `
            <article class="respuesta">
                1 usd = ${valor} ${monedaEquivalencia}
            </article>`;
